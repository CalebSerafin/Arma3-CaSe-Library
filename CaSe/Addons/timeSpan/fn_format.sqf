...
Example:
    ...
    [DEV_timeSpan]                      call CaSe_fnc_timeSpan_format;  // "(-) 21 Minutes 69 Microseconds 420 Nanoseconds"
    [DEV_timeSpan,1]                    call CaSe_fnc_timeSpan_format;  // "(-) 21m 69µs 420ns"
    [DEV_timeSpan,2,2]                  call CaSe_fnc_timeSpan_format;  // "-0:0:21:0–0:69:420"

    // Zeros. Note the negative marker in DEV_timeSpan.
    DEV_timeSpan = [true];
    [DEV_timeSpan]                      call CaSe_fnc_timeSpan_format;  // "(Now)"
    [DEV_timeSpan,0,0,true]             call CaSe_fnc_timeSpan_format;  // "(+) (Now)"
    [DEV_timeSpan,1]                    call CaSe_fnc_timeSpan_format;  // "0"
    [DEV_timeSpan,1,0,true]             call CaSe_fnc_timeSpan_format;  // "(+) 0"
    [DEV_timeSpan,0,2,false]            call CaSe_fnc_timeSpan_format;  // "0 Days 0 Hours 0 Minutes 0 Seconds 0 Milliseconds 0 Microseconds 0 Nanoseconds"
    [DEV_timeSpan,0,2,false,nil,true]   call CaSe_fnc_timeSpan_format;  // "00 Days 00 Hours 00 Minutes 00 Seconds 000 Milliseconds 000 Microseconds 000 Nanoseconds"
    [DEV_timeSpan,1,2,false]            call CaSe_fnc_timeSpan_format;  // "0d 0h 0m 0s 0ms 0µs 0ns"
    [DEV_timeSpan,2,2,false]            call CaSe_fnc_timeSpan_format;  // "0:0:0:0–0:0:0"
    [DEV_timeSpan,2,2,true,nil,true]    call CaSe_fnc_timeSpan_format;  // "+00:00:00:00–000:000:000"

    // Field visibility.
    DEV_timeSpan = [false,0,3,54,0,152,0];
    [DEV_timeSpan,0]                    call CaSe_fnc_timeSpan_format;  // "3 Hours 54 Minutes 152 Milliseconds"
    [DEV_timeSpan,0,1]                  call CaSe_fnc_timeSpan_format;  // "3 Hours 54 Minutes 0 Seconds 152 Milliseconds"
    [DEV_timeSpan,0,2]                  call CaSe_fnc_timeSpan_format;  // "0 Days 3 Hours 54 Minutes 0 Seconds 152 Milliseconds 0 Microseconds 0 Nanoseconds"
    [DEV_timeSpan,0,2,true]             call CaSe_fnc_timeSpan_format;  // "(+) 0 Days 3 Hours 54 Minutes 0 Seconds 152 Milliseconds 0 Microseconds 0 Nanoseconds"

    // Slicing.
    DEV_timeSpan = [false,0,3,54,0,152,0];
    [DEV_timeSpan,0,1,false,2]          call CaSe_fnc_timeSpan_format;  // "3 Hours 54 Minutes"
    [DEV_timeSpan,0,2,false,[1,1e7]]    call CaSe_fnc_timeSpan_format;  // "3 Hours 54 Minutes 0 Seconds 152 Milliseconds 0 Microseconds 0 Nanoseconds"
    [DEV_timeSpan,0,2,false,[0,4]]      call CaSe_fnc_timeSpan_format;  // "0 Days 3 Hours 54 Minutes 0 Seconds"
    [DEV_timeSpan,0,2,false,[1,4]]      call CaSe_fnc_timeSpan_format;  // "3 Hours 54 Minutes 0 Seconds"

    // Slicing to get digital time.
    DEV_timeSpan = [false,0,3,54,0,152,0];
    [DEV_timeSpan,2,2,false,[1,4]]      call CaSe_fnc_timeSpan_format;  // "3:54:0"
    [DEV_timeSpan,2,2,false,[1,4],true] call CaSe_fnc_timeSpan_format;  // "03:54:00"

    // Localised all places on machine
    DEV_timeSpan = [false,1,2,3,4,5,6,7];
    [nil,0,0,false,nil,false,true]          call CaSe_fnc_timeSpan_format;  // "(Now)"
    [DEV_timeSpan,0,2,false,nil,false,true] call CaSe_fnc_timeSpan_format;  // "1 Days 2 Hours 3 Minutes 4 Seconds 5 Milliseconds 6 Microseconds 7 Nanoseconds"
    [DEV_timeSpan,1,2,false,nil,false,true] call CaSe_fnc_timeSpan_format;  // "1d 2h 3m 4s 5ms 6µs 7ns"
*/

// CaSe_fnc_timeSpan_format = {
...
private _sizeFieldList = if (_localise && (_symbolSet != 2)) then {
    ...
    [
        ["STR_case_timeSpan_days","STR_case_timeSpan_hours","STR_case_timeSpan_minutes","STR_case_timeSpan_seconds","STR_case_timeSpan_milliseconds","STR_case_timeSpan_microseconds","STR_case_timeSpan_nanoseconds"],
        ["STR_case_timeSpan_days_abbr","STR_case_timeSpan_hours_abbr","STR_case_timeSpan_minutes_abbr","STR_case_timeSpan_seconds_abbr","STR_case_timeSpan_milliseconds_abbr","STR_case_timeSpan_microseconds_abbr","STR_case_timeSpan_nanoseconds_abbr"]
    ...
    if (_x != 0 || _showInBetweenZeros && _foundNonZero || _showAllZeros) then {
        ...
        if (_pad) then {
            if (_forEachIndex < 4) then {
                _amount = _amount call _fnc_pad_2Digits;
            } else {
                _amount = _amount call _fnc_pad_3Digits;
            };
        };
...
if (_formattedText isEqualTo "") then {
    if (_localise && (_symbolSet == 0)) then {
        _formattedText = (localize "STR_case_timeSpan_now") + " ";
    } else {
